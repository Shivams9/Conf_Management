...
def delete_conference(id):
    try:
        ...
        # Optionally handle related feedback records
        feedbacks = Feedback.query.filter_by(conference_id=id).all()
        for feedback in feedbacks:
            db.session.delete(feedback)

        db.session.delete(conference)
        db.session.commit()
        flash('Conference and related feedback successfully deleted.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'An error occurred while trying to delete the conference: {e}', 'danger')
